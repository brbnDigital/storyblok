...
async function GenerateTypes(options) {
    if (!variables_1.ACCESS_TOKEN || !variables_1.SPACE_ID)
        return Promise.reject("Storyblok Access Token and Space ID are required");
    let generatorOptions = { ...DEFAULT_OPTIONS, ...options };
    let groupUUIDs = {};
    let components = [];
    let schema = [];
    let output = "";
    let client = new storyblok_js_client_1.default({ oauthToken: variables_1.ACCESS_TOKEN }, variables_1.STORYBLOK_API);
    await Generate();
    /**
    ...
    async function GetAllComponentGroups() {
        let data = await client.get(`spaces/${variables_1.SPACE_ID}/component_groups`);
    ...
    async function GetAllComponents() {
        let data = await client.get(`spaces/${variables_1.SPACE_ID}/components`);
